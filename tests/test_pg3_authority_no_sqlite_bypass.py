...
def _install_fake_state(monkeypatch, tmp_path, *, authority: bool):
    ...
    # upstream 은 SQLite 핸들을 hermes_state_registry.acquire 로 공유한다 — 가짜 SessionDB 로 위임.
    hsr = types.ModuleType("hermes_state_registry")
    hsr.acquire = lambda db_path=None: SessionDB(db_path=db_path)
    monkeypatch.setitem(sys.modules, "hermes_state_registry", hsr)
    return calls
