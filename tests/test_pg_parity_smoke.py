...
* Every assertion below runs against a real PostgreSQL container through the
  live ``SessionDB`` API — none of them are stubs. Where a case asserts parity,
  the same operation is performed on both backends and the observable results
  are compared, rather than freezing an expected value.
"""
...
def pg_clean(pg_dsn):
    ...
    with psycopg.connect(pg_dsn, autocommit=True) as conn:
        conn.execute(
            ...
            "session_turn_leases, state_meta, schema_version, "
            "pg_migration_version CASCADE"
        )
    yield pg_dsn
...
def test_harness_container_roundtrip(pg_dsn):
    ...
    RETURNING + advisory lock all work. This is NOT a parity assertion — it
    isolates harness failures from real backend failures, so a broken container
    setup cannot be mistaken for a backend bug.
    """
    with psycopg.connect(pg_dsn, autocommit=True) as conn:
        cur = conn.cursor()
        cur.execute(
...
# -- Parity assertions: SQLite result == PostgreSQL result --------------------
#
# Each asserts SQLite-result == PG-result on an identical fixture, exercised
# through the real backend-agnostic SessionDB API. Coverage is scoped to the
# divergent idioms that actually appear in the SQL this class emits: LIKE
# case-sensitivity, the integer `active` flag filter, and json_extract reads.
# The empty-string-vs-NULL and integer-division divergences have no occurrences
# in that SQL, so no assertion guards them.
...
def test_b1_trgm_migration_applies(pg_clean, tmp_path):
    ...
    with psycopg.connect(pg_clean, autocommit=True) as conn:
        # Both v17 (EXTENSION) and v18 (indexes) must be recorded. Postgres-only
        # migrations are tracked in pg_migration_version, which is deliberately
        # separate from the shared schema_version — sharing one MAX(version)
        # made every pg-only migration look applied once the shared version
        # climbed past them.
        for version in (17, 18):
            row = conn.execute(
                "SELECT 1 FROM pg_migration_version WHERE version = %s", (version,)
            ).fetchone()
            assert row is not None, (
                f"pg_migration_version missing v{version} after migration"
            )
